...
- `spec.event_shock is not None` (8 event-based specs): a new `build_hypothesis_event_scenario(spec, base_scenario) -> ScenarioConfig` — one `ShockEvent(day=<fixed day>, type=ShockType(spec.event_shock), magnitude=<TBD>, target_currency=spec.event_target_currency)` appended ON TOP of ALL of `base_scenario`'s existing shocks, unmodified (additive, not replacing `build_sandbox_scenario`'s "drop the currency-targeted ones first" convention — that convention exists because `build_sandbox_scenario`'s currency-targeted shocks name SYNTHETIC symbols absent from a sandbox's universe entirely; hypothesis-sims use real currencies, so `apply_currency_shock`/`TrustLedger.update` already no-op safely on anything outside this hypothesis's restricted set, and dropping them anyway would make an event cell strictly less eventful than its own baseline variant instead of "baseline plus one new event" — see New info.pdf's own framing of this section: "run through a depeg event and see how the attitudes of the agents change", which calls for isolating one new event's marginal effect against an otherwise-unchanged baseline). Shock day and magnitude picked to match existing conventions (`build_sandbox_scenario`'s `_DEPEG_MAGNITUDE = 0.15` precedent for `DEPEG_EVENT`; a comparable value for `BANK_FAILURE` from `master_simulation.yaml`'s own existing entry) — exact values are an implementation-time detail, not a design fork.
