...
Immediately after a cell/seed/utility_type's day-loop completes (env still in memory, checkpoint just deleted per §0's "accept restart" decision — no new checkpointing for this phase):
...
- **H2-H11** (the other 20 specs): for each `EquivalenceComparison` in `EQUIVALENCE_COMPARISONS[spec.hypothesis]` (1 row for H3-H11, 2 for H2), call `cohort_indifference_points(env, comparison, model_id, client)`, persist one `IndifferencePointRecord` row per cohort.
