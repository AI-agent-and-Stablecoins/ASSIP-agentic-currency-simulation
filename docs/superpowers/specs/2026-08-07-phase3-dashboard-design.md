...
- **Live progress (day counts, decision counts)**: direct read-only SQL queries against the project's SQLite database (`database/session.py`'s `DATABASE_URL`), scoped by the tracked `matrix_run_id`. Specifically: `MAX(timestep)` per `run_id` from `timestep_logs` (current day), row counts from `llm_decisions` filtered by `run_id`. WAL mode (already enabled, Plan 6a) makes concurrent reads safe alongside the simulation's own writes, in both single-process and distributed modes. **No transaction count**: `database/models.py`'s `TransactionRecord` has no run-scoping column at all (its `buyer_id`/`seller_id` are the same unscoped agent-id strings that collide across cells sharing a seed — the same root cause the Plan 6a schema fix addressed for `agents`/`wallets`, but `transactions` itself was never fixed since nothing read it before this dashboard). Adding that column is a real schema change and explicitly out of scope for this plan (user decision, 2026-08-07) — revisit as a separate, properly-reviewed fix if a transaction count is wanted later.
...
- Adding a run-scoping column to `database/models.py`'s `TransactionRecord` (§2) — a real schema change, not "purely additive dashboard tooling." A transaction count is simply not shown.
