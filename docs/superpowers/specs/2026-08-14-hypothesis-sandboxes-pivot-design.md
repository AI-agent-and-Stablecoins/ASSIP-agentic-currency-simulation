...
**Amendment (found during whole-plan review, implemented as part of Task 1): "offered on chain X" is realized via per-currency chain pinning, not a restricted chain universe.** Restricting `env.chains` to just `{ethereum, solana}` would NOT force the tradeoff — an agent could still pick the better currency on the cheaper of those two chains, dodging the tradeoff entirely. Instead, `generate_candidates` (`src/blockchain/routing_engine.py`) gained an optional `currency_chain_pins: dict[str, str] | None` parameter: when a symbol is a key in that dict, it only ever pairs with its pinned chain; every other symbol still pairs with every chain in the (unrestricted, all-4-chain) universe. `Environment` carries this as a settable `currency_chain_pins` attribute (default `{}`), read by both `generate_candidates` call sites in `src/simulation/timestep.py`. This also has a second benefit the original design didn't anticipate: `load_currency_universe()`'s full chain set includes Arbitrum and Base too, which a naive `env.chains` restriction would have excluded entirely from every hypothesis-sim, not just the gas-fee ones.

**Amendment: hypothesis-sims must run with `use_llm=True`.** `CRRAUtility`/`CARAUtility`/`EpsteinZinProxyUtility` (`src/utility/`) are each a strictly monotone transform of a single effective-wealth scalar, so the rule-based deterministic choice path's argmax is identical across every risk-aversion cohort — risk-aversion only differentiates an agent's choice on the real LLM decision path, where `risk_aversion` is printed into that agent's own prompt context. Running a hypothesis-sim deterministically would silently produce four identical cohort rows with no error anywhere. Recorded here (and in `src/economy/hypothesis_scenarios.py`'s module docstring) as an explicit, binding decision for whichever future task builds the runner — not a code enforcement mechanism yet, since no runner exists in this plan's scope to enforce it in.

**Amendment: hypothesis-population agents are exempt from loss-driven risk adaptation.** `src/economy/risk_adaptation.py`'s `adapt_cara_coefficient` (invoked during persistence) mutates any agent whose `cara_coefficient` is non-`None`, converting its `utility_type`/`risk_aversion` via `cara_utility_fields` and capping the result at `configs/economy/risk_adaptation_params.yaml`'s `a_max` (5.0) — below this plan's own `a=6.0` cohort, and liable to silently convert a CRRA/Epstein-Zin agent onto CARA utility mid-run. Both would violate this plan's core "one fixed utility function and risk-aversion cohort for the whole sim" reporting guarantee. `generate_hypothesis_population` (`src/agents/population.py`) sets `cara_coefficient = None` on every cohorted agent after construction, so `adapt_cara_coefficient`'s own existing guard skips them entirely — deliberately relying on the pre-existing no-op-for-`None` contract rather than adding a new one.

**Note for sub-project B (found during whole-plan review, not yet actioned):** because `cara_coefficient` is `None` for every hypothesis-sim agent, `AgentStateRecord.cara_coefficient` (the column `src/econometrics/hypothesis_datasets.py`'s `attach_cara_a` currently joins on) will be NULL for all of them — that join would silently return an empty dataframe, not an error. Whichever future task reads a hypothesis-sim's per-agent risk-aversion cohort back out of the database must read it from `LLMDecisionRecord.utility_parameters["risk_aversion"]` instead (populated for every LLM-path decision, per Fix 5's `use_llm=True` requirement) — not from `cara_coefficient`, and not from `AgentRecord`/`AgentStateRecord`'s `risk_profile` string (a role-level label, identical across all four cohorts).

Wallet seeding for every hypothesis-sim uses `seed_restricted_wallets` (`src/economy/wallet_seeding.py`, extracted from the original `_seed_sandbox_wallets` during the whole-plan review — same split-by-USD-value logic, moved to a shared, lightweight module `matrix_runner.py`'s existing 6 sandboxes now also import from via a compatibility alias).
